...
public class Main {
	...
	public static void main(String[] args) throws JsonIOException, IOException {
		...
		List<Referencia> referencias = extrair(6, 2017);
	...
	private static List<Referencia> extrair(int mes, int ano) {
		...
		for (String uf : ufs) {
			...
			try {
				...
				logger.error("Falha ao processar o arquivo " + url, e);
			}
	...
	public static void parse(String file, Referencia referencia) throws IOException {
	...
	private static void downloadFile(String urlFormatada, String toFile) {
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Baixando: " + urlFormatada);
				logger.debug("Destino: " + toFile);
			}
